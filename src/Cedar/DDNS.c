...
DDNS_CLIENT *NewDDNSClient(CEDAR *cedar, UCHAR *key, INTERNET_SETTING *t)
{
	...
	// ComfyConnect: SoftEther-hosted Dynamic DNS is disabled. Returning NULL keeps
	// DDnsClient absent everywhere -- all consumers null-check it, b_support_ddns
	// reports false, and the config saver writes "Disabled: true". No phone-home.
	return NULL;
